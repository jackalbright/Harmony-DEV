 ...
 */
	function startup(&$controller) {
		...
		error_log("AUTH_STARTUP");

		if ($loginAction == $url) {
			error_log("IN LOGIN $loginAction");
			$model =& $this->getModel();
			if (empty($controller->data) || !isset($controller->data[$model->alias])) {
				error_log("NO DATA OR USER DATA");
				error_log("SESS_CHECK AUTH_REDIR (will ignore new redir)=".!$this->Session->check('Auth.redirect'));
				if (!$this->Session->check('Auth.redirect') && !$this->loginRedirect && env('HTTP_REFERER')) {
					error_log("CHANGING GOTO TO REFERER=".env("HTTP_REFERER"));
 ...
 */
	function redirect($url = null) {
		error_log("AUTH REDIR URL=$url");
		if (!is_null($url)) {
			$redir = $url;
			$this->Session->write('Auth.redirect', $redir);
			error_log("SET AUTH REDIR URL=$redir");
		} elseif ($this->Session->check('Auth.redirect')) {
			$redir = $this->Session->read('Auth.redirect');
			error_log("GET AUTH REDIR URL=$redir");
			$this->Session->delete('Auth.redirect');

			if (Router::normalize($redir) == Router::normalize($this->loginAction)) {
				$redir = $this->loginRedirect;
				error_log("NORMALIZE LOGREDIR=$redir");
			}
		} else {
			$redir = $this->loginRedirect;
			error_log("ELSE LOGREDIR=$redir");
		}
