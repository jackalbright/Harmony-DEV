...
<div id="<?= $sid ?>" class="jcarousel_container" style="<?= empty($images) ? "display:none;":"" ?>">
	...
	<li>
		...
		j('#<?= $imgid ?>').click(function() {
			...
			// If there is a compatibility version, use that instead. as it may be a PDF, etc.
			//var filename = '<?= !empty($image['CustomImage']['Image_Location']) ? $image['CustomImage']['Image_Location'] : '/'.$image['CustomImage']['path'].'/'.$image['CustomImage']['filename'] ?>';
			
			var filename = '<?= !empty($image['CustomImage']['display_location']) ? $image['CustomImage']['display_location'] : '/'.$image['CustomImage']['path'].'/'.$image['CustomImage']['filename'] ?>';
			/*
			<?php
			//print_r($image['CustomImage']);
			?>
			*/
