...
?>
	<h4>Text/Quotation</h4>
	<div style="background: #DDD;">

	<table width="100%">
	<tr><td> 
		<?= $this->Form->input("quote_font", array('label'=>'Font','options'=>$fonts,'default'=>$defaultFont)); ?>
	</td><td align='left'>
		<?= $this->Form->input("quote_font_size", array('label'=>'Size','options'=>$quoteFontSizes,'default'=>$defaultFontSize)); ?>
	</td></tr></table>
	...
	<table width="100%">
	<tr><td> 
		<?= $this->Form->input("personalization_font", array('label'=>'Font','options'=>$persFonts,'default'=>$defaultPersFont)); ?>
	</td><td align='left'>
		<?= $this->Form->input("personalization_font_size", array('label'=>'Size','options'=>$persFontSizes,'default'=>$defaultPersFontSize)); ?>
	</td></tr></table>
...
j('#DesignQuote_clone').bind('preview', function() {
	if(j(this).is(":visible"))
	{
		...
		j('#fullbleed #quote p:first-child').html(string).addClass('ghosted').parent().contain(5);
	}
});
...
j('#DesignQuoteStyleCenter, #DesignQuoteStyleDropcap').bind('preview', function() {
	// IE doesnt quite like :first-letter with em's

	var quote = j('#fullbleed #quote').html();
	var dropcap = j('#DesignQuoteStyleDropcap').is(":checked");

	if(j(this).is(":checked"))
	{
		var capchar = quote.substr(0,1);
		quote = quote.substr(1);

		quote = "<span>"+capchar+"</span>"+quote;
	} else {
		quote = j('#fullbleed #quote').text();
	}
	j('#fullbleed #quote').html(quote);
	j('#fullbleed #quote').toggleClass('center', !dropcap);
});
