...
<? if($coord['id'] == 'fullbleed') { ?>
.overlays, <? } # Overlay must match coords. ?>
.<?= $coord['id'] ?>
{
	position: absolute;
	top: <?= intval($coord['y']); ?>px;
	left: <?= intval($coord['x']); ?>px;
	width: <?= ceil($coord['width']) ?>px;
	height: <?= ceil($coord['height']); ?>px;
...
.<?= $coord['id'] ?>
{
	...
	<? if(!in_array($coord['id'], array('personalization'))) { ?>
		width: <?= ceil($coord['width']); ?>px;
	<? } ?>
	<? if($coord['id'] == 'personalization') { ?>
		left: <?= intval($coord['x'] - $fullbleed[0]['x'] + $coord['width']/2); ?>px;
		max-height: <?= ceil($items['fullbleed']['height']/3); ?>px;
	<? } else if($coord['id'] == 'quote') { # slight clip with 'max-height' ?>
		left: <?= intval($coord['x'] - $fullbleed[0]['x']); ?>px;
		max-height: <?= ceil($items['fullbleed']['height']); ?>px;
	<? } else { ?>
		left: <?= intval($coord['x'] - $fullbleed[0]['x']); ?>px;
		height: <?= ceil($coord['height']); ?>px;
	<? } ?>
...
.fullbleed .quote.dropcap p.text.<?=$font_id?> span.cap
{
	...
	*/
	...
	XXX fix dropcapDrop
	*/
	line-height: <?= (!empty($fontTweaks[$font_id]['dropcapLineHeightFudge']) ? $fontTweaks[$font_id]['dropcapLineHeightFudge'] : $textOptions['dropcapLineHeightFudge']) ?>em;

	margin-right: <?= (!empty($fontTweaks[$font_id]['dropcapMarginRight']) ? $fontTweaks[$font_id]['dropcapMarginRight'] : $textOptions['dropcapMarginRight'])*
		(!empty($fontTweaks[$font_id]['dropcapMarginRightFudge']) ? $fontTweaks[$font_id]['dropcapMarginRightFudge'] : 1) * 3.75;# Fudge factor. 
		?>em;
	margin-top: <?= (!empty($fontTweaks[$font_id]['dropcapMarginTop']) ? $fontTweaks[$font_id]['dropcapMarginTop'] : $textOptions['dropcapMarginTop'])*4 ?>em;
}
