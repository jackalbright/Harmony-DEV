			...
			<tr>
				...
				<div id="AddToCart" class='cta' align="right">
						<? if(!empty($design['Design']['cart_item_id'])) { ?>
							<?= $this->Form->submit("/images/buttons/Update-Cart.gif",array('onClick'=>"j.loading(); return j('#DesignForm').assertMinimum();",'div'=>false)); ?>
						<? } else { ?>
							<?= $this->Form->submit("/images/buttons/Add-to-Cart.gif",array('onClick'=>"j.loading(); return j('#DesignForm').assertMinimum();",'div'=>false)); ?>
						<? } ?>
					<div class='clear'></div>
				</div>
				</td>
