...
<div id="<?= $side ?>_step_image" class='side<?=$side?>'>
	...
	<?= $this->Form->input("CustomImage.$side.filename", array('type'=>'hidden','value'=>$image_src,'class'=>'DesignSideFilename')); ?>
...
j('#CustomImage<?=$side?>Filename').bind('preview', function() {
	...
	if(filename && filename != placeholder)
	{
		...
		//console.log(j('#<?= $side ?>_parts').position());
		//console.log(j(img));
		j(img).show().removeClass('placeholder').addClass('enabled');
