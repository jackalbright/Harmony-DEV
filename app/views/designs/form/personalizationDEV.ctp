<h3 id="<?=$side?>_header_personalization" class='step_header side<?=$side?>'><?=$i ?>. Type Additional Text (optional)</h3>
<div id="<?=$side?>_step_personalization" class='step side<?=$side?>'>
	<?= !empty($part['part_summary']) ? "<p class='tip'>".$part['part_summary']."</p>" : "" ?>
<?
###############################
...
<?= $this->Form->hidden("DesignSide.$side.personalization_width", array('id'=>"{$side}_personalization_width",'div'=>false,'label'=>false,'size'=>4)); ?>
...
j('#DesignSide<?=$side?>PersonalizationFont').bind('preview', function() {
	...
	j('#preview<?=$side?> .fullbleed .personalization p').removeClass(allfonts).addClass(font).parent().trigger('updated');
...
j('#DesignSide<?=$side?>Personalization').bind('reset', function() {
	j('#<?= $side?>_personalization_x, #<?= $side ?>_personalization_y, #<?= $side?>_personalization_width').val(''); // clear  coords.
	// Now re-draw text where it ought to be...
	j('.fullbleed #<?= $side?>_personalization').css({top: '', left: '', width: ''}).trigger('updated');
	j(this).trigger('preview');
});

j('#DesignSide<?=$side?>Personalization').bind('preview', function() {
	j('#preview<?= $side?> .fullbleed .personalization').initialize_centerize();

	// ghost isnt active, custom value was set.
	if(!j(this).val())// && j(this).closest('.ui-accordion-content').hasClass('completed')) 
	{
		j('#preview<?=$side?> .fullbleed .personalization p').html('').removeClass('ghosted').hide().parent().removeClass('enabled').contain(5);
		...
		var string = j(this).val().nl2br();
		j('#preview<?= $side?> .fullbleed .personalization p').show().html(string).removeClass('ghosted').parent().centerize().addClass('enabled').contain(5);
	}
	j('#preview<?=$side?> .fullbleed .personalization').delayTrigger(function() { j('#preview<?=$side?> .fullbleed .personalization').trigger('updated'); }, 500); // 'this' is not available!
});
...
j('#DesignSide<?=$side?>PersonalizationNone').bind('preview', function() {
	...
	if(none)
	{
		j('#preview<?=$side?> .fullbleed .personalization p').hide().parent().trigger('updated');
...
j('#DesignSide<?=$side?>PersonalizationFontSize').bind('preview', function() {
	...
	j('#preview<?=$side?> .fullbleed .personalization').css({fontSize: fontSize+"px"}).trigger('updated');
});

j('#DesignSide<?=$side?>PersonalizationColor').bind('preview', function() {
	var color = j(this).val();
	j('#preview<?=$side?> .personalization').css({color: color}).trigger('updated');
});
...
// XXX initialize personalization to center, then keep centered as typing...

j('#side<?=$side ?>').bind('visible', function() { // things sensitive to position.
	//j('#preview<?=$side?> .personalization').initialize_centerize().enableDraggable().draggable_overlay();//.resizable().centerize();
	j('#preview<?=$side?> .personalization').enableDraggable().draggable_overlay();//.resizable().centerize();
});
</script>
