...
j('#DesignSide<?=$side?>Personalization').bind('preview', function() {
	//j('#preview<?= $side?> .fullbleed .personalization').initialize_centerize(); // copied by Jack from DEV version 4-1-14
	...
	if(!j(this).val())// && j(this).closest('.ui-accordion-content').hasClass('completed')) 
	{
		...
		var string = j(this).val().nl2br();
		j('#preview<?= $side?> .fullbleed .personalization p').show().html(string).removeClass('ghosted').parent().addClass('enabled').contain(5);
	}
});
