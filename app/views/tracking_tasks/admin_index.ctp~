<div class="trackingTasks index">
<h2><?php __('TrackingTasks');?></h2>
<p>
<?php
echo $paginator->counter(array(
'format' => __('Page %page% of %pages%, showing %current% records out of %count% total, starting on record %start%, ending on %end%', true)
));
?></p>
<table cellpadding="0" cellspacing="0">
<tr>
	<th><?php echo $paginator->sort('tracking_task_id');?></th>
	<th><?php echo $paginator->sort('tracking_area_id');?></th>
	<th><?php echo $paginator->sort('sort_index');?></th>
	<th><?php echo $paginator->sort('name');?></th>
	<th><?php echo $paginator->sort('url');?></th>
	<th><?php echo $paginator->sort('description');?></th>
	<th class="actions"><?php __('Actions');?></th>
</tr>
<?php
$i = 0;
foreach ($trackingTasks as $trackingTask):
	$class = null;
	if ($i++ % 2 == 0) {
		$class = ' class="altrow"';
	}
?>
	<tr<?php echo $class;?>>
		<td>
			<?php echo $trackingTask['TrackingTask']['tracking_task_id']; ?>
		</td>
		<td>
			<?php echo $html->link($trackingTask['TrackingArea']['name'], array('controller'=> 'tracking_areas', 'action'=>'view', $trackingTask['TrackingArea']['tracking_area_id'])); ?>
		</td>
		<td>
			<?php echo $trackingTask['TrackingTask']['sort_index']; ?>
		</td>
		<td>
			<?php echo $trackingTask['TrackingTask']['name']; ?>
		</td>
		<td>
			<?php echo $trackingTask['TrackingTask']['url']; ?>
		</td>
		<td>
			<?php echo $trackingTask['TrackingTask']['description']; ?>
		</td>
		<td class="actions">
			<?php echo $html->link(__('View', true), array('action'=>'view', $trackingTask['TrackingTask']['tracking_task_id'])); ?>
			<?php echo $html->link(__('Edit', true), array('action'=>'edit', $trackingTask['TrackingTask']['tracking_task_id'])); ?>
			<?php echo $html->link(__('Delete', true), array('action'=>'delete', $trackingTask['TrackingTask']['tracking_task_id']), null, sprintf(__('Are you sure you want to delete # %s?', true), $trackingTask['TrackingTask']['tracking_task_id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
</table>
</div>
<div class="paging">
	<?php echo $paginator->prev('<< '.__('previous', true), array(), null, array('class'=>'disabled'));?>
 | 	<?php echo $paginator->numbers();?>
	<?php echo $paginator->next(__('next', true).' >>', array(), null, array('class'=>'disabled'));?>
</div>
<div class="actions">
	<ul>
		<li><?php echo $html->link(__('New TrackingTask', true), array('action'=>'add')); ?></li>
		<li><?php echo $html->link(__('List Tracking Areas', true), array('controller'=> 'tracking_areas', 'action'=>'index')); ?> </li>
		<li><?php echo $html->link(__('New Tracking Area', true), array('controller'=> 'tracking_areas', 'action'=>'add')); ?> </li>
	</ul>
</div>
