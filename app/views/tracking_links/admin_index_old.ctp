<div class="trackingLinks index">
<h2><?php __('TrackingLinks');?></h2>
<p>
<?php
echo $paginator->counter(array(
'format' => __('Page %page% of %pages%, showing %current% records out of %count% total, starting on record %start%, ending on %end%', true)
));
?></p>
<table cellpadding="0" cellspacing="0">
<tr>
	<th><?php echo $paginator->sort('id');?></th>
	<th><?php echo $paginator->sort('referer');?></th>
	<th><?php echo $paginator->sort('url');?></th>
	<th><?php echo $paginator->sort('name');?></th>
	<th><?php echo $paginator->sort('x');?></th>
	<th><?php echo $paginator->sort('y');?></th>
	<th><?php echo $paginator->sort('session_id');?></th>
	<th><?php echo $paginator->sort('created');?></th>
	<th><?php echo $paginator->sort('modified');?></th>
	<th class="actions"><?php __('Actions');?></th>
</tr>
<?php
$i = 0;
foreach ($trackingLinks as $trackingLink):
	$class = null;
	if ($i++ % 2 == 0) {
		$class = ' class="altrow"';
	}
?>
	<tr<?php echo $class;?>>
		<td>
			<?php echo $trackingLink['TrackingLink']['id']; ?>
		</td>
		<td>
			<?php echo $trackingLink['TrackingLink']['referer']; ?>
		</td>
		<td>
			<?php echo $trackingLink['TrackingLink']['url']; ?>
		</td>
		<td>
			<?php echo $trackingLink['TrackingLink']['name']; ?>
		</td>
		<td>
			<?php echo $trackingLink['TrackingLink']['x']; ?>
		</td>
		<td>
			<?php echo $trackingLink['TrackingLink']['y']; ?>
		</td>
		<td>
			<?php echo $trackingLink['TrackingLink']['session_id']; ?>
		</td>
		<td>
			<?php echo $trackingLink['TrackingLink']['created']; ?>
		</td>
		<td>
			<?php echo $trackingLink['TrackingLink']['modified']; ?>
		</td>
		<td class="actions">
			<?php echo $html->link(__('View', true), array('action' => 'view', $trackingLink['TrackingLink']['id'])); ?>
			<?php echo $html->link(__('Edit', true), array('action' => 'edit', $trackingLink['TrackingLink']['id'])); ?>
			<?php echo $html->link(__('Delete', true), array('action' => 'delete', $trackingLink['TrackingLink']['id']), null, sprintf(__('Are you sure you want to delete # %s?', true), $trackingLink['TrackingLink']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
</table>
</div>
<div class="paging">
	<?php echo $paginator->prev('<< '.__('previous', true), array(), null, array('class'=>'disabled'));?>
 | 	<?php echo $paginator->numbers();?>
	<?php echo $paginator->next(__('next', true).' >>', array(), null, array('class' => 'disabled'));?>
</div>
<div class="actions">
	<ul>
		<li><?php echo $html->link(__('New TrackingLink', true), array('action' => 'add')); ?></li>
	</ul>
</div>
