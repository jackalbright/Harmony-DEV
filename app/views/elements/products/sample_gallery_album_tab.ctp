...
?>
	...
	<tr>
		<td style="width: 40px;">
			<a href="Javascript:void(0)" onClick="return image_gallery_album_scroll_left('<?=$id?>_row');"><img src="/images/buttons/Circle-left.gif"/></a>
		</td>
		<td align=center>
			<div id="<?= $id ?>_row">
				<? 
				...
				{ 
					...
					#
				?>
				<a class="image <?= $i++ > 0 ? 'hidden' : "" ?> lightbox image_gallery_scroll_image sample_image_<?=$underpath?>" title="<?= $image['title'] ?>" rel="shadowbox[<?=$underpath?>]" id="image_gallery_largelink1_<?=$underpath?>" href="/images/galleries/<?= $path ?>/<?= $image['product_image_id'] ?>.<?=$image['file_ext'] ?>">
		...
		<td style="width: 40px;">
			<a href="Javascript:void(0)" onClick="return image_gallery_album_scroll_right('<?=$id?>_row');"><img src="/images/buttons/Circle-right.gif"/></a>
		</td>
	</tr>
	</table>
