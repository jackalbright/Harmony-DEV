...
?>
	...
	<div class="preview">
		...
		<div align="center">
			...
			<br/>
				<a href="/custom_images?prod=<?= $product['Product']['code'] ?>?start_over=1" onClick='showPleaseWait();'><img src="/images/webButtons2014/orange/large/browseStamps.png"/></a>
			<? } else { ?>
				<a href="/products/design/<?= $product['Product']['code'] ?>?start_over=1" onClick='showPleaseWait();'><img src="/images/webButtons2014/green/large/startDesigningMyOwn.png"/></a>
			<? } ?>
		</div>
		<br/>
		<? } ?>
	...
	?>
		...
		<? foreach($thumbs[$pid] as $imgid => $thumburl) { 
			...
			$alt = !empty($alts[$imgid]) ? $alts[$imgid] : null;
		?>
		<div class="thumb <?= $i++ == 0 ? "selected":"" ?>">
			<a title='<?= $alt ?>' id="thumb_<?= $imgid ?>" href="<?= $previewurl ?>" rel="<?= $largerurl ?>">
				<img alt='<?= $alt ?>' src="<?= $thumburl ?>"/>
			</a>
		</div>
		<? } ?>
		<div class="clear"></div>
