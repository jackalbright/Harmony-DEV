...
<div style="text-align: right;">
	...
	<?
	echo $this->element("products/create_button", array('product'=>$product['Product'],'list_link'=>1)); 
	if ($position == 'bottom')
	{
	?>
	...
	if(isset($subProducts) && count($subProducts)) {
		foreach($subProducts as $subProduct) {
			...
			{
				?> <div class="border padded margin_bottom_small"> <?
				echo $hd->product_element("products/create_button", $product['Product']['prod'], array('product'=>$subProduct));
				if ($position == 'bottom')
				{
				?>
