...
?>
	...
	function setLayout(layout, load)
	{
		...
		*/
		...
		var oldLayout = j('#template').val();

		if(oldLayout == layout) { return; } // Do nothing.
...
<div style="display: none; padding: 10px;">
	...
	<? if(empty($product_config['image.2']) && empty($product_config_fullview['image.2'])) { ?>
		...
		if(empty($standard_options_map['quote']) && (!empty($standard_options_map['personalization']) && !in_array($build['Product']['code'], array('RL'))))
		{
		?>
		<td valign="bottom" width="200" align="center">
			<?= 
