<div style="border: solid #CCC 1px; background-color: white; padding: 2px; padding-bottom: 50px; position: relative;">
<div class="clear"></div>
...
?>
					...
					<h2><?= $counter ?>. 
						<?
							if (!empty($step_info["Part"]))
							{
								if (!empty($step_info['Part']['part_title']))
								{
									echo $step_info['Part']['part_title'];
								} else {
									echo "Choose your " . strtolower($step_info['Part']['part_name']);
								}
								if($step_info['ProductPart']['optional'] == 'yes') { echo " <span class='note'>(Optional)</span>"; }
							} else {
								echo ucwords($current_step);
								echo " <span class='note'>(Optional)</span>";  #Other stuff is optional. ie 'comments'
							}
					...
					<div align="right" class="">
						...
						<? } else { ?>
							<input type="image" name="action" src="/images/buttons/Next.gif" onClick="showPleaseWait();"/>
						<? } ?>
					</div>
					<? 
