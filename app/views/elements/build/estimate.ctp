		...
		<div class="preview_option_quantity preview_option_value">
			...
			<? if(!empty($build['quantity_price_list'])) { ?>
				...
				foreach($build['quantity_price_list'] as $option => $option_cost)
				{
					...
					if($option == 'stamp') { $option_name = (in_array($build['Product']['code'], array('ST','P')) ? 'Surcharge <span style="font-size: 10px;">(High Value Stamp)</span>' : 'Surcharge <span style="font-size: 10px;">(Genuine Stamp)</span>'); }
				?>
				<tr>
					<td> 
						<? if ($option_cost < 0) { ?>
				...
				<tr>
					...
					<td  style="border-top: solid #CCC 1px;"align="right"> <?= sprintf("$%.02f", $build['quantity_price_list']['total']); ?> </td>
				</tr>
				<? } ?>
			<? } ?>
			<tr>
				...
				<td valign="middle" align="right">
					x <input id="setupCharge" type="hidden" name="data[setup_charge]" value="<?= !empty($build['Product']['setup_charge']) ? $build['Product']['setup_charge'] : 0 ?>"/>
					<input style="text-align: right; width: 3em;" id="quantity" type="text" name="data[quantity]" value="<?= !empty($build['quantity']) ? $build['quantity'] : $build['Product']['minimum']; ?>" size="4"/>
					<br/>
					<a id="update_button" href="Javascript:void(0);" onClick="if(assertMinimum('<?= $build['Product']['minimum'] ?>')) { update_build_pricing(); }"><img align="top" alt="Calculate" src="/images/buttons/small/Calculate-grey.gif"/></a>
				</td>
			</tr>
			<tr>
				<td class="bold">
				...
				<? if (empty($is_wholesale) && $discounted) { ?>
					<div id="estimate_retail" style="text-decoration: line-through;"><?= sprintf("$%.02f", $build['quantity'] * $build['retail_price_list']['total']); ?></div>
				<? } ?>
					<div id="estimate_total" style="color: #B82A2A;"><?= sprintf("$%.02f", (!empty($build['proof_only']) ? $proof_cost : $build['quantity'] * $build['quantity_price_list']['total'])); ?></div>
				<? if (empty($is_wholesale) && $discounted) { ?>
