...
<div class="sidebar_block nomodal" id="">
<table width="100%">
<tr>
<td width="50%" valign="top">
	...
	<tr>
		<td colspan=3 valign="top">
			...
			<div class="right">
				<input type="image" align="top" src="/images/webButtons2014/grey/small/calculate.png" onClick="showPleaseWait();"/>
			</div>
            ...
            <p>Other charges may apply if there are surcharges, added features, or upgrades for each particular product (eg. double-sided printing)</p>
            </div>
...
<td valign="top" style="border-left: solid #CCC 1px; padding: 5px; padding-top: 35px;">
	...
	<? if($product['Product']['is_stock_item']) { ?>
	<form action="/cart/add.php" target='_top' class='nomodal'>
		<? if($prod == 'CH') { ?>
		...
		<input type="image" src="/images/webButtons2014/orange/large/addToCart.png"/>
	</form>
	<? } else { ?>
	<form action="/gallery" target='_top' class='nomodal'>
		<input id="atc_prod" type="hidden" name="prod" value="<?= $prod ?>"/>
		<input id="atc_quantity" type="hidden" name="quantity" value="<?= !empty($quantity) ? $quantity : $product['Product']['minimum'] ?>"/>
		<? if(in_array($prod, array('ST','P'))) { ?>
			<input type="image" src="/images/webButtons2014/orange/large/browseStamps.png"/>
		<? } else { ?>
