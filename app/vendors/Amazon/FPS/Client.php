...
class Amazon_FPS_Client implements Amazon_FPS_Interface
{
    ...
    private  $_config = array (
    				'ServiceURL' => 'https://fps.amazonaws.com',
    				'SandboxServiceURL' => 'https://fps.sandbox.amazonaws.com',
    ...
    {
        $fpsServiceEndPoint = defined("AWS_SANDBOX") ? $this->_config["SandboxServiceURL"] : $this->_config["ServiceURL"];
        $query = http_build_query($parameters);

        //initialize CURL
        $curlHandle = curl_init();

	error_log("CURL=$fpsServiceEndPoint");
        ...
        curl_setopt($curlHandle, CURLOPT_CAINFO, dirname(__FILE__).'/../ca-bundle.crt');
        curl_setopt($curlHandle, CURLOPT_CAPATH, dirname(__FILE__).'/../ca-bundle.crt');
	...
	if(empty($response))
	{
		error_log("CURL_ERROR=".print_r(curl_error($curlHandle),true));
	}

        // to grab the response code only from the Header
	error_log("RESPONSE (not two lines)=".print_r($response,true));
    ...
    private function _calculateStringToSignV2(array $parameters) {
        $data = 'POST';
        $data .= "\n";
        $endpoint = parse_url (defined("AWS_SANDBOX") ? $this->_config['SandboxServiceURL'] : $this->_config['ServiceURL']);
        $data .= $endpoint['host'];
        $data .= "\n";
