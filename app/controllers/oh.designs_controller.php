...
{
	var $uses = array('CustomImage','CartItem','Quote','ProductQuote','GalleryCategory','OrderItem','Part');
	...
	var $fontTweaks = array( 
		...
		'tk-caliban-std'=>array( # FF,IE,SVG
			...
			'dropcapMarginTop'=>-0.04, # neg means move up; only used on web, not in SVG
		),
		...
		'tk-leander-script-pro'=>array( # FF,IE,SVG
			...
			'dropcapFactor'=>1.75,
			'dropcapDrop'=>1.05,
			'rightShaveFudge'=>0.3, # Good enough...
		...
		'tk-poetica-std'=>array( # FF,IE,SVG
			...
			'dropcapMarginRight'=>0.25,
			'dropcapMarginRightFudge'=>0.25,
			'dropcapLineHeightFudge'=>0.6,
			'dropcapMarginTop'=>0.05,
		),
	...
	function add($prod = null)
	{
		...
		if(!empty($sides))
		{
			...
			{
				$imgid = !empty($side['customImageID']) ? 
					$side['customImageID'] : null;
	...
	function form($prod = null) # Stuff needed for form
	{
		...
		$orient = $this->Session->read("Design.Design.orientation");
	...
	function load_order_design($order_item_id)
	{
		...
		{
			$image = $this->CustomImage->read(null, $design['DesignSide'][1]['customImageID']);
			$design['CustomImage'][1] = $image['CustomImage'];
		}
		if(!empty($design['DesignSide'][2]['customImageID']))
		{
			$image = $this->CustomImage->read(null, $design['DesignSide'][2]['customImageID']);
			$design['CustomImage'][2] = $image['CustomImage'];
		}
	...
	function draw($side = 1) # Generate SVG
	{
		...
		if(!empty($this->params['named']['cart_item_id'])) 
		{
			$design = $this->load_cart_design($this->params['named']['cart_item_id']);

		# XXX load order_item_id for order sheets...
		...
		$prod = !empty($design['Design']['productCode']) ? $design['Design']['productCode'] : "B";
		$prod = $this->productCode($prod); 
		# May be using PARENT svg files...
		$svgpath = "$basepath/products/$prod.svg";
		...
		if(!empty($design))
		{
			...
			if(empty($filename) && !empty($i['Image_Location'])) { 
			...
			$this->set_image($svg, "border1", (!empty($s['border_id']) && $s['border_id'] > 0) ? $meta['borderImages'][$s['border_id']] : null, null, !empty($s['border1_y']) ? $s['border1_y'] : null);
			$this->set_image($svg, "border2", (!empty($s['border_id']) && $s['border_id'] > 0) ? $meta['borderImages'][$s['border_id']] : null, null, !empty($s['border2_y']) ? $s['border2_y'] : null);
			...
			if(!empty($this->params['named']['print']))
			{
				...
				$this->set_image($svg, "tassel", !empty($s['tassel_id']) ? $meta['tasselImages'][$s['tassel_id']] : null);
			...
			if(!empty($s['quote']))
			{
				...
				list($quoteEndX,$quoteEndY) = $this->set_text($svg, "quote", $s['quote'],
					$quote_font, $quote_font_size, $quote_x, $quote_y, $quote_style, $quote_color);
				...
				if(!empty($attribution))
				{
					...
					$this->set_text($svg, "quote", $attribution,
						...
						$attribution_x, $attribution_y, 'attribution', $quote_color);
				}
			} else { # Clear.
			...
			if(!empty($s['personalization']))
			{
				...
				$this->set_text($svg, "personalization", $s['personalization'], $personalization_font, $personalization_font_size, $personalization_x, $personalization_y, 'center', $personalizationColor);
			} else { # clear
