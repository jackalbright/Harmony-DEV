...
class FaqTopicsController extends AppController {
	...
	function admin_resort()
	{
		...
		if(!empty($this->params['form']['faqTopics']))
		{
			$order = $this->params['form']['faqTopics'];

			foreach($order as $ix => $faq_topic_id)
			{
				$this->FaqTopic->id = $faq_topic_id;
				error_log("SAVING TOPIC $faq_topic_id TO $ix");
				$this->FaqTopic->saveField("sort_index", $ix);
			}
		}
		exit(0);
