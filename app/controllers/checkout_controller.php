...
class CheckoutController extends CartItemsController {
	...
	var $complete = array();
	var $public_methods = array('form','shipping_options','update_shipping_speed','form_address_edit','form_address_select','form_address_view','form_address_delete','form_data','admin_save_cart_item','redeem_coupon','thankyou');
	var $logged = false;
	...
	/*--------------------------
	function: thankyou 
	synopsis: This function redirects to the form() function but allows us to isolate the page that the user lands on when the purchase is complete.
	This is to facilitate AdRoll tracking.
	
	Returns: redirects to the form function 
	--------------------------*/
	function thankyou(){
		//return $this->redirect(array('action'=>'receipt'));
	...
	function form() # one-pager.
	{
		...
		if($this->require_wholesale() && !empty($this->data)) # Process order.
		{
			...
			*/
			...
			// The following commented-out lines were from the thank you page testing when we were trying to change the URL of the thankyou page
			// leave them in, in case we decide to revisit that project
			
			//$testVars = json_encode($_POST);
			//$this->Session->write("testVars", $testVars);
			//$this->controller = 'checkout'; // added by Jack....remove if it breaks anything 
			...
			//$this->redirect("/checkout/thankyou"); // JSA does not do what I thought it did
	...
	function _finalize_purchase($purchase_id, $cart_items, $customer_id = null, $session_id = null)
	{
		...
		foreach($cart_items as $cart_item)
		{
			...
			$orientation = !empty($cart_item['CartItem']['orientation']) ? $cart_item["CartItem"]['orientation'] : null;
			...
			foreach($sides as $side) # one or two sides, ie item_parts
			{
				...
				$item_part = array(
					...
					# need to save other NEWER details.....
						...
						'quote_width'=>!empty($side['quote_width']) ? $side['quote_width'] : null,
						...
						'personalization_width'=>!empty($side['personalization_width']) ? $side['personalization_width'] : null,
						'border1_x'=>!empty($side['border1_x'])?$side['border1_x']:null, 
						'border2_x'=>!empty($side['border2_x'])?$side['border2_x']:null, 
						'border1_y'=>!empty($side['border1_y'])?$side['border1_y']:null, 
						'border2_y'=>!empty($side['border2_y'])?$side['border2_y']:null, 
				);
				
				$testCartValue = json_encode($item_part);
				$this->Session->write("testCartValue",$testCartValue);
