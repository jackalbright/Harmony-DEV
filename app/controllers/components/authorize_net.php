<?php 
App::import('Vendor','authorizenet.cim.class.php');
class AuthorizeNetComponent extends Object {
	...
	$shippinginfo	: Associative Array containing values for customer shipping details
	$purchase_id	: Reference id for order
	...
	*/
	...
	# REALLY should be modified to allow for flexible input....
	
		// setup variables
		$ccexp = $ccexpmonth . '/' . $ccexpyear;


		error_log("CHARGE_CARD=$invoice");
		...
		$dup_window = 10; # SHUT OFF, so declined ones don't get approved by accident the second time around....
		
		$authnet_values				= array
		(
			"x_duplicate_window"		=> $dup_window,
			"x_invoice_num"			=> $invoice,
			"x_test_request"		=> $live ? false : true,
		...
		#echo "RESP=".print_r($responsearray,true);
		
		return $responsearray;
		
	} // end chargeCard function

	function createCIMAccount($customer, $card, $billing, $shipping)
	{
		...
		$cim->setParameter('cardNumber', $cardModel->decrypt($card['Number']));
		$cim->setParameter('expirationDate', date('Y-m', strtotime($card['Expiration'])));
		...
		$profileId = $cim->customerProfileId;
