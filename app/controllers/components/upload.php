<?php
	...
	class UploadComponent extends Object {
		...
		function upload ($file, $destination, $name = NULL, $rules = NULL, $allowed = NULL) {
			...
			#if (!is_null($allowed)) { $this->_allowed = $allowed; } else { $this->_allowed = array('jpg','jpeg','gif','png','pjpg','pjpeg','tif','psd','pdf'); }
			if (!is_null($allowed)) { $this->_allowed = $allowed; } else { $this->_allowed = array('jpg','jpeg','gif','png','pjpg','pjpeg'); }
			...
			if (isset($file) && is_array($file) && !$this->upload_error($file['error'])) {
				...
				if(!in_array($this->ext(strtolower($fileName)),$this->_allowed)){
					$this->error("Please resave your file in one of the following formats and upload again: ". strtoupper(join(", ", $this->_allowed)) );
				} else { 
				
					// -- it's been uploaded with php
					if (is_uploaded_file($fileTmp)) {
						error_log("UPFILE=$fileTmp");
						error_log("IS_THERE=".file_exists($fileTmp));
						...
						if ($rules == NULL) {
							...
							if (move_uploaded_file($fileTmp, $output)) {
								...
								$this->error("Could not move '$fileTmp' to '$fileName'");
							}
						} else {
							// -- gd lib check
