...
class PhpPsdReader {
	...
	function PhpPsdReader($fileName) {
		...
		error_log("READ END: ".time());
	}


	function getImage() {
		error_log("GET_IMAGE START: ".time());
		...
		error_log("L3A: ".time());
		error_log("ROWS=".$this->infoArray['rows']);
		error_log("COLS=".$this->infoArray['columns']);
